...
- `runs/<timestamp>-<slug>/plan.md`: 当前计划
- `runs/<timestamp>-<slug>/memory.md`: 完整工作记忆
- `runs/<timestamp>-<slug>/messages/worker-XX.md`: 每轮 worker 最新消息
- `runs/<timestamp>-<slug>/messages/reviewer-XX.md`: 每轮 reviewer 意见
- `runs/<timestamp>-<slug>/state.json`: 当前状态机快照
